...
print_access_info() {
    ...
    echo ""
    echo "[ACCESS] Access Information:"
    echo "====================="
    echo "[JAEGER] Jaeger UI:           http://localhost:16686"
    echo "[PROMETHEUS] Prometheus:          http://localhost:9090"
    echo "[GRAFANA] Grafana:             http://localhost:3000 (admin/atonix2024!)"
    echo "🚨 Alertmanager:        http://localhost:9093"
    echo "[LOKI] Loki:               http://localhost:3100"
    echo "[OTEL] OpenTelemetry UI:   http://localhost:55679"
    echo "[ELASTIC] Elasticsearch:      http://localhost:9200"
    echo "[KIBANA] Kibana:             http://localhost:5601"
    echo ""
    echo "[CONFIG] Configuration:"
    echo "=================="
    echo "[BACKEND] Backend config:      .env.observability"
    echo "[FRONTEND] Frontend config:     frontend/.env.local"
    echo "[STACK] Stack config:        infrastructure/observability/"
    echo ""
...
main() {
    ...
    echo ""
    log_success "[SUCCESS] OpenTelemetry setup completed successfully!"
}
